...
with tab1:
    ...
    st.write("""
    Imagine you're playing a game where the outcome is uncertain. A random variable is like a special rule that assigns a number to each possible outcome of this game. It's a way to quantify uncertainty!

    For example, if you're rolling a die, the random variable could be "the number that shows up on top". Each roll can result in a number from 1 to 6, and each number has a certain probability of occurring.
    """)
    
    st.subheader("Types of Random Variables")
    col1, col2 = st.columns(2)
    with col1:
        st.info("**Discrete Random Variable**: It's like counting things. The values are distinct and separate.\n\n🎭 Example: Number of people in a theater (0, 1, 2, 3, ...)")
    with col2:
        st.info("**Continuous Random Variable**: It's like measuring things. The values can be any number within a range.\n\n🌡️ Example: Temperature outside (20.1°C, 20.15°C, 20.153°C, ...)")
...
with tab2:
    ...
    st.write("""
    A discrete random variable is like a choosy counter. It only picks from a specific set of values, usually whole numbers.
    Think of it as counting things: people, cars, or stars in the sky.
    """)
    
    st.subheader("🎲 Interactive Example: Rolling Dice")
    st.write("Let's roll some dice and see what happens!")
...
with tab3:
    ...
    st.write("""
    A continuous random variable is like a super precise measurement tool. It can take any value within a range, 
    even tiny fractions that are hard to count. Think of it as measuring things: height, weight, or time.
    """)
    
    st.subheader("📏 Interactive Example: Height Distribution")
    st.write("Let's explore the distribution of adult heights!")
    ...
    st.write("""
    🔍 Hover over the chart to see exact values. The colored areas represent different percentile ranges.
    The red dashed line shows the mean height.
    """)

    # Interactive height checker
